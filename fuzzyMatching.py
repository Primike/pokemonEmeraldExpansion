...
import os
import time
from thefuzz import process, fuzz # Import fuzzy matching library
...
REQUEST_DELAY = 0.5
SIMILARITY_THRESHOLD = 88 # Adjust this value (0-100) for fuzzy matching sensitivity
...
# Convert set to list for fuzzy matching function
ALLOWED_MOVES_LIST = list(ALLOWED_MOVES)
...
def format_c_move_name(move_name):
    """
    Performs basic formatting on fetched move name for initial comparison.
    Handles a few specific known name variations NOT based on similarity.
    """
    ...
    for row in rows:
        ...
        if level_cell and move_cell:
            ...
            if move_name_tag:
                ...
                except ValueError: level = 1 if level_text=='1' or level_text.lower()=='evo.' or level_text=='—' else -1 # Use -1 for unhandled? Or skip? Skip for now.
...
def update_learnset_in_file(file_path, pokemon_name, moves_data):
    ...
    Assumes moves_data contains dicts with {'level': L, 'move': 'Fetched Name', 'matched_allowed_move': 'MOVE_CORRECT_NAME'}
    """
    ...
    for move_info in moves_data:
        ...
        c_move_name = move_info.get('matched_allowed_move')
        if c_move_name: new_move_lines.append(f"{INDENT}LEVEL_UP_MOVE({level:>2}, {c_move_name}),\n")
        else: print(f"      INTERNAL ERROR: Missing 'matched_allowed_move' for {pokemon_name} move {move_info.get('move')} at level {level}. Skipping line.")

    new_lines = lines[:start_index + 1] + new_move_lines + [f"{end_marker_line_exact}\n", lines[brace_index]] + lines[brace_index + 1:]

    try:
        if lines == new_lines:
            print(f"      Learnset for {c_pokemon_name} already up-to-date (with fuzzily matched Gen 1-3 moves).")
        else:
            with open(file_path, 'w', encoding='utf-8') as f: f.writelines(new_lines)
            print(f"      Successfully updated {c_pokemon_name}'s learnset in {file_path} (with fuzzily matched Gen 1-3 moves).")
        return True
...
if __name__ == "__main__":
    ...
    print(f"--- Applying Gen 1-3 Move Filter ({len(ALLOWED_MOVES)} moves) using Fuzzy Matching (Threshold: {SIMILARITY_THRESHOLD}) ---")
    ...
    print("Ensure 'thefuzz' and 'python-Levenshtein' are installed (`pip install thefuzz python-Levenshtein`)")


    fetch_failures = []
    update_failures = []
    all_fuzzy_matches_applied = [] # <--- Initialize list to store all fuzzy match details
    processed_count = 0
        ...
        for pokemon_name in pokemon_list:
            ...
            if fetched_moves is None:
                ...
                if not fetched_moves:
                    ...
                    # Filter moves using fuzzy matching
                    filtered_moves = []
                    disallowed_moves_found = []
                    # fuzzy_matches_made_this_pokemon = [] # Optional: Track per pokemon

                    for move_info in fetched_moves:
                        formatted_name = format_c_move_name(move_info['move'])
                        match = process.extractOne(formatted_name, ALLOWED_MOVES_LIST, scorer=fuzz.WRatio)
                        best_match_name, score = (match if match else (None, 0))

                        if best_match_name and score >= SIMILARITY_THRESHOLD:
                            move_info['matched_allowed_move'] = best_match_name
                            filtered_moves.append(move_info)
                            # Log fuzzy matches IF the name wasn't identical
                            if formatted_name != best_match_name:
                                 # Add details to the main list for final summary
                                 match_details = {
                                     "pokemon": pokemon_name,
                                     "generation": gen,
                                     "fetched_move": move_info['move'],
                                     "formatted_name": formatted_name,
                                     "matched_move": best_match_name,
                                     "score": score
                                 }
                                 all_fuzzy_matches_applied.append(match_details)
                                 # fuzzy_matches_made_this_pokemon.append(f"'{move_info['move']}' ({formatted_name}) -> '{best_match_name}' ({score})")
                        else:
                            disallowed_moves_found.append(f"{move_info['move']} ({formatted_name}, best fuzzy: {best_match_name} score {score})")

                    print(f"      Filtered Moves: Kept {len(filtered_moves)} out of {total_fetched} fetched moves (based on fuzzy match >= {SIMILARITY_THRESHOLD}).")
                    # Optional: Print fuzzy matches per pokemon
                    # if fuzzy_matches_made_this_pokemon:
                    #     print(f"      Fuzzy matches applied this Pokémon: {', '.join(fuzzy_matches_made_this_pokemon)}")
    ...
    # Print All Fuzzy Matches Applied
    if all_fuzzy_matches_applied:
        print(f"\n--- Fuzzy Matches Applied Summary ({len(all_fuzzy_matches_applied)} instances) ---")
        print("Review these matches where similarity >= threshold but names weren't identical:")
        # Sort for consistent output (optional)
        all_fuzzy_matches_applied.sort(key=lambda x: (x['pokemon'], x['generation'], x['fetched_move']))
        for match in all_fuzzy_matches_applied:
            print(f"  - {match['pokemon']} (Gen {match['generation']}): Fetched '{match['fetched_move']}' ({match['formatted_name']}) matched to -> '{match['matched_move']}' (Score: {match['score']})")
    else:
        print("\nNo fuzzy matches needed to be applied (all matches were either exact or below threshold).")